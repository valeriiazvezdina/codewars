...
function digitalRoots(n) {
    ...
    if (arr.length > 1) {
        let newArr = arr.reduce((prevVal, item) => prevVal + +item, 0);
        return digitalRoots(newArr)
        } else return arr.reduce((prevVal, item) => prevVal + +item, 0)
}
console.log(digitalRoots(493193))